...
unsafe extern "system" fn WndProc(hwnd: HWND,
                                  message: UINT,
                                  wparam: WPARAM,
                                  lparam: LPARAM)
                                  -> LRESULT {
    ...
    unsafe {
        let hwnd_edit =
            user32::CreateWindowExW(0,
                                    ...
                                    ptr::null(),
                                    ES_MULTILINE | ES_AUTOVSCROLL | ES_READONLY | WS_VISIBLE |
                                    WS_CHILD | WS_VSCROLL |
                                    WS_HSCROLL,
                                    0,
                                    0,
...
fn initialize_window() -> Result<(), String> {
    ...
    let edit = try!(create_richedit(instance, window)
        .map_err(|e| format!("Error creating richedit: {}", e)));
    ...
    unsafe {
        ...
        loop {
            match user32::GetMessageW(&mut msg, ptr::null_mut(), 0, 0) {
                -1 => {
                    utils::msgbox(&format!("Error in the message loop: {}",
